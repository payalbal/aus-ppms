...
rdata_path <- file.path(ppm_path, "RData") ## *** NOTE: all files within to be recreated after BB's landuse maps (currently using regSSP_birds files)
output_path <- "./output"
...
## Test correlations in covariates - run once & saved
# covariates <- readRDS(file.path(rdata_path, "covariates_aus.rds"))
# covariates_subset <- covariates[[-which(names(covariates)%in%covs_bias)]]
# cov_df <- getValues(covariates_subset)
# cov_df <- na.omit(cov_df)
# preds <- colnames(correlations(cov_df))
# saveRDS(preds, file = file.path(output_path, "preds_aus.rds"))
# rm(cov_df, covariates_subset)

## Covaritaes for prediction, write into temp folder structure - ****NOTE: update based on scearios used for BB's lu maps
ssps <- paste0("ssp", 1:3)
rcps <- c("45", "60", "85")
...
## Stack covariates ...see regSSP_birds: sdm_maxent.R
## Present
...
all(names(covs_pred) == names(covs_mod)) # check that names are the same 

## Biodiversity data
occdat <- readRDS(file.path(ppm_path, "ala_data/ala_data.rds"))
occdat <- occdat[,.(decimalLongitude, decimalLatitude, scientificName)] # ** order required for ppm function
names(occdat)[3] <- c("species")
...
fit_ppms_apply <- function(i, spdat, bkdat, bkwts, interaction_terms, ppm_terms, species_names, n.fits=50, min.obs = 50) {
  ...
  # spxy_landuse_extract <- extract(covs_mod[[1]], spxy, method = 'simple') ## gives NAs
  ...
  spxyz <- extract(covs_mod[[-1]], spxy, buffer = 1000000, small = TRUE, 
                   fun = mean, na.rm = TRUE)
...
# temp <- fit_ppms_apply(1:length(spp), spdat, bkdat, bkwts, interaction_terms, ppm_terms, species_names = spp, n.fits=100, min.obs = 50)
